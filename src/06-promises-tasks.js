...
async function processAllPromises(array) {
  const arr = await Promise.all(array);
  return arr;
}
...
async function getFastestPromise(array) {
  const arr = await Promise.race(array);
  return arr;
}
...
async function chainPromises(array, action) {
  ...
  return arr.then((resolvedArr) => {
    resolvedArr.forEach((el) => {
      el.then((val) => result.push(val));
    });
  })
    .then(() => result.reduce((a, b) => action(a, b)));
}
