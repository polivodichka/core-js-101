...
function getPolynom(...args) {
  return (x) => {
    ...
    Object.values(args).forEach((num) => {
      if (power > 1) answer.push(num * x ** power);
      else if (power === 1) answer.push(num * x);
      else if (power === 0) answer.push(num);
      power -= 1;
    });
...
function retry(func, attempts) {
  ...
  return () => {
    while (attempt <= attempts) {
      try { return func(); } catch (e) { attempt += 1; }
    }
    return false;
  };
}
